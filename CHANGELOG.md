...
- **Hydrus database scan (hash tier)** — a new scan mode that works on files already in Hydrus instead of a folder on disk. Hydrus supplies each file's MD5 without transferring a byte, so the existing four-booru exact-hash fan-out runs unchanged and writes tags, source URLs, and notes straight back onto the file. Selection is a Hydrus query — file domain, `system:number of tags < N`, images only, inbox/archive, free-form predicates, and a `system:limit` cap applied last so the budget is spent on files that passed every filter. Budgets for wall-clock time and consecutive failures, plus a dry-run mode that writes nothing. Perceptual matching is deliberately not part of this mode.
- **Deleted duplicates are looked up for the file you kept** — a kept file no booru recognises often shares a duplicate group with a deleted file they do. Hydrus keeps the MD5s of files it no longer stores, so those hashes are free extra lookups whose results belong to the surviving file. Only relationship `8` (duplicate) is followed — alternates are deliberately different artwork, so their tags would be wrong. Members still present are excluded, and recovered files land on the Duplicate Tagged page. Off-by-default for files that already matched; configurable to always.
- **Scan source selector in the GUI** — the Scan tab now chooses between a folder and the Hydrus database; picking the database swaps the folder picker for the scan's selection, duplicate, bookkeeping, and budget options, with a live preview of the exact Hydrus query. Progress, issues, log, and Cancel are shared with folder scans; the perceptual card and the folder-only actions are hidden because this mode has no use for them. Starting an uncapped, non-dry run asks for confirmation first.
- **Scan bookkeeping lives in Hydrus, not a ledger** — a database scan marks each file `furtag:scanned` plus `furtag:matched`/`furtag:nomatch` in a configurable tag service, and puts `-furtag:scanned` in the next run's query. That makes a capped scan walk the database across runs instead of re-examining its first N files, and unlike a ledger it is visible, searchable, and undoable from inside Hydrus. A file whose lookup errored is left unmarked so the next run retries it, mirroring how the folder pipeline refuses to seal a partial hit. Each run also writes a JSONL report plus a text summary beside `settings.json`.
- **Credentials no longer leak into the log** — `requests` puts the whole request URL in its exception text, so a Gelbooru or SauceNAO failure printed the live API key verbatim into the issue pane, and into any screenshot of it. `notify()` — the single point every user-facing message passes through — now masks every known secret value, so a new source cannot forget to. `redact_secrets()` already existed and was tested, but nothing had ever called it. **Rotate any Gelbooru or SauceNAO key that has appeared in a FurTag log.**
- **A dropped Gelbooru connection retries instead of costing the lookup** — Gelbooru closes TLS connections mid-handshake often enough under the concurrent hash tier that a lone `SSLEOFError` says nothing about the file; the same MD5 usually succeeds on the next attempt. Transport failures now get a second attempt, paced by the existing lane pacer. Only transport failures: a response the server actually sent is a real answer and is raised as it stands. When both attempts fail the lane pauses 10s, for the same reason Fluffle's 5xx path backs off, and the file stays retryable.
- **Indexing does ~86% fewer syscalls** — `has_sidecar`, `_directory_manifest`, and `local_path_complete` now answer "does this sidecar exist?" from the directory listing the walk already read, instead of stat-ing each of the six candidate names per media file. Measured on a 155,092-file library on an external USB volume: 1,085,970 stat calls to 155,418, and 143.0s to 126.9s warm. The manifest digest is unchanged bit-for-bit, so existing directory seals stay valid. Per-directory recomputation of the search-profile, output-policy, and direct-notes digests was also hoisted out of the walk. The remaining cost is `os.walk` itself (~57s of that 127s), which this does not address.
- **Indexing reports progress** — a long index showed a static "Indexing…" in the GUI, indistinguishable from a hang. It now reports folders scanned and media found as it goes, through the same observer the scan uses.
- **Launch is no longer blocked by the last folder** — FurTag restores the most recently scanned folder on startup but no longer indexes it immediately. Indexing disables Browse, the drop target, Recent, Reset, and the whole Settings panel, so on a large library the window arrived unusable and stayed that way until discovery finished. The restored folder now waits behind a new **Index** button beside Browse; picking a folder yourself (Browse, drop, or Recent) still indexes right away, since that is an explicit choice.
