...
- **Database-scoped checkpoints** — Hydrus completion state is bound to a persisted, non-secret database identity plus API origin. FurTag fingerprints the live library from durable local file service keys, so pointing the same API address at a different Hydrus database reopens old Hydrus checkpoints automatically, and switching back restores the previous identity. **Hydrus → Database identity → Reset…** force-revalidates only the currently bound database.
